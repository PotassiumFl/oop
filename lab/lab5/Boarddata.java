...
public class Boarddata {
    ...
    public Boarddata(int height, int width, int boardNum, String playerBlack, String playerWhite) {
        ...
        board.add(new Board(height, width, board.size(), Rule.gomoku, playerBlack, playerWhite));
    }
    ...
    public void boardGame(Scanner input) {
        ...
        if (this.board.get(this.currentBoard - 1).ifFinished == 1)
        ...
        for (Player player : this.board.get(this.currentBoard - 1).player) {
            if (player.color == this.board.get(this.currentBoard - 1).currentColor) {
                if (this.board.get(this.currentBoard - 1).canPass[player.color.ordinal()] == 1
                        ...
                        && this.board.get(currentBoard - 1).rule == Rule.reversi)
                    System.out.println("玩家[" + player.name + "]无子可下！");
                System.out.print("请玩家[" + player.name + "]输入");
                if (this.board.get(this.currentBoard - 1).ifFinished == 0)
                    System.out.print("落子位置（1A）/");
                System.out.print("棋盘编号（1-" + this.boardNum + "）/新游戏类型（peace或reversi或gomoku）");
        ...
        while (ifCorrect == 0) {
            ...
            if (move.matches("\\d+")) {// 全数字，则为棋盘跳转
            ...
            } else if (move.length() == 2 && this.board.get(this.currentBoard - 1).ifFinished != 1) {// 长为2且游戏未结束，进入切割
                ...
                this.boardNum += 1;
                return;
            } else if (move.equals("gomoku")) {
                this.board.add(new Board(this.height, this.width, this.board.size(), Rule.gomoku, this.playerBlack,
                        this.playerWhite));
                this.boardNum += 1;
                return;
